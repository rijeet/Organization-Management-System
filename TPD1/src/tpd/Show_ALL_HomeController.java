 ...
 * @author Mukaffi
 */
public class Show_ALL_HomeController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            Show_ALL_HomeController.HomeList = dbAction.getAllOldAgeHome();
        } catch (SQLException ex) {
            Logger.getLogger(Show_ALL_HomeController.class.getName()).log(Level.SEVERE, null, ex);
        }
        for (OldAgeHome_Houses old : Show_ALL_HomeController.HomeList) {
    ...
    private void delete(ActionEvent event) throws SQLException {
      ObservableList<OldAgeHome_Houses> selectedhome=FXCollections.observableArrayList();
        selectedhome=HomeTable.getSelectionModel().getSelectedItems();
        DatabaseAction dbAction=new DatabaseAction();
        dbAction.deletehome(selectedhome);
       HomeList.removeAll(selectedhome);
