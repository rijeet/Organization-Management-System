 ...
 * @author Mukaffi
 */
public class Show_ALL_StudentController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Show_ALL_StudentController.studentList.clear();
        Student_Table.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);
        DatabaseAction dbAction = new DatabaseAction();
        try {
            Show_ALL_StudentController.studentList = dbAction.getAllStudent();
        } catch (SQLException ex) {
            Logger.getLogger(Show_ALL_StudentController.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void Delete_button(ActionEvent event) throws SQLException {
        ...
        selectedStudent = Student_Table.getSelectionModel().getSelectedItems();
        DatabaseAction dbAction = new DatabaseAction();
        dbAction.deleteStudent(selectedStudent);
        Show_ALL_StudentController.studentList.removeAll(selectedStudent);
