...
public class Show_All_SchoolController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            Show_All_SchoolController.schoolList = dbAction.getAllSchool();
        } catch (SQLException ex) {
            Logger.getLogger(Show_All_SchoolController.class.getName()).log(Level.SEVERE, null, ex);
        }
        for (School old : Show_All_SchoolController.schoolList) {
    ...
    @FXML
    private void DeleteButton_Action(ActionEvent event) throws SQLException {
        ObservableList<School> selectedschool = FXCollections.observableArrayList();
        selectedschool = School_Table.getSelectionModel().getSelectedItems();
        DatabaseAction dbAction = new DatabaseAction();
        dbAction.deleteSchool(selectedschool);
        schoolList.removeAll(selectedschool);
